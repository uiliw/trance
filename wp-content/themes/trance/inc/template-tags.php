...
function understrap_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'trance' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);
	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'trance' ),
...
function understrap_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'trance' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
function custom_mini_cart() {
    ...
    echo '<div class="dropdown dropdown-cart-button">';
    echo '<a href="#" class="dropdown-back" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> ';
    ...
    echo '</div>';
    echo '</a>';
    echo '<ul class="dropdown-menu  animation slideUpIn dropdown-menu-right dropdown-menu-mini-cart">';
