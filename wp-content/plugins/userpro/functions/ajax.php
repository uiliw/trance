		...
		switch($ajaxcheck) {
			...
			case 'email_exists':
				if (!is_email($input_value)) {
                    ...
                    $domains = array();
                    $userpro_allowed_email_domains = userpro_get_option('userpro_allow_email_domains');
                    $userpro_block_email_domains = userpro_get_option('userpro_block_email_domains');
                    if( !empty( $userpro_allowed_email_domains ) ){

                        $userpro_allowed_email_domains = preg_replace('/\s+/', '',$userpro_allowed_email_domains);
                        $domains = explode(',',$userpro_allowed_email_domains);
                        if(!in_array($domain, $domains)) $domain_flag = '1';

                    }else if(!empty($userpro_block_email_domains)){

                        $userpro_block_email_domains = preg_replace('/\s+/', '',$userpro_block_email_domains);
