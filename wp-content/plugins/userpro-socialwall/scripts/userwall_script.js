...
function userwall_postdislikecount_post(post_id,userid,totaldislikes,totalliks,cdate) {
	...
	str = 'action=socialwall_dislikecount_post&post_id='+post_id+'&userid='+userid;
		...
		success:function(data){
                    ...
                    if(data === 'yes'){
                        jQuery('#userwall_postlikecount_post'+post_id).html('<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-up"></i><i class="socialwall_postlikecount_post ">'+totalliks+'&nbsp</i><i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-down"></i><i class="socialwall_postlikecount_post ">'+total+'</i><i onclick="userwall_report_post('+post_id+' ,'+userid+')" class="reportpost fa fa-exclamation-circle" style="margin-left: 12px;color: red;"></i><span class="commented-date" style="color: #90949c;font-family: Arial;font-size:12px;margin-left: 1%;">'+date+'</span>');
                    }
                    else if(data === 'No'){ 
                        jQuery('#userwall_postlikecount_post'+post_id).html('<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-up"></i><i class="socialwall_postlikecount_post ">'+totalliks+'&nbsp</i><i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-down"></i><i class="socialwall_postlikecount_post ">'+total+'</i><i style="color:black;opacity: 0.5;cursor:default;margin-left: 12px;" class="fa fa-exclamation-circle"></i><span class="commented-date" style="color: #90949c;font-family: Arial;font-size:12px;margin-left: 1%;">'+date+'</span>');
                    } 
		},
		error:function(data){alert(data);
	...
	str = 'action=socialwall_count_posts_like&post_id='+post_id+'&userid='+userid;
		...
		success:function(data){
			...
			jQuery('#userwall_postlikecount_post'+post_id).show();
                        ...
                        if(data === 'yes'){
                            jQuery('#userwall_postlikecount_post'+post_id).html('<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-up"></i><i class="socialwall_postlikecount_post ">'+total+'&nbsp</i><i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-down"></i><i class="socialwall_postlikecount_post ">'+totaldislike+'</i><i onclick="userwall_report_post('+post_id+' ,'+userid+')" class="reportpost fa fa-exclamation-circle" style="margin-left: 12px;color: red;"></i><span class="commented-date" style="color: #90949c;font-family: Arial;font-size:12px;margin-left: 1%;">'+date+'</span>');
                        }
                        else if(data === 'No'){
                            jQuery('#userwall_postlikecount_post'+post_id).html('<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-up"></i><i class="socialwall_postlikecount_post ">'+total+'&nbsp</i><i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-down"></i><i class="socialwall_postlikecount_post ">'+totaldislike+'</i><i style="color:black;opacity: 0.5;cursor:default;margin-left: 12px;" class="fa fa-exclamation-circle"></i><span class="commented-date" style="color: #90949c;font-family: Arial;font-size:12px;margin-left: 1%;">'+date+'</span>');
                        }
...
function userwall_commentlikedislikecount_comment(meta_id,userid,totallikes,totaldislikes,action,postid,commentdate)
{
    var m_names = new Array("Jan", "Feb", "Mar", 
        ...
        var curr_hrs = d.getHours();
        var curr_min = d.getMinutes();
        var comment_date = curr_date + "-" + m_names[curr_month] + "-" + curr_year;
	...
	jQuery.ajax({
		...
		success:function(data){
			...
			jQuery('#userwall_commentlikecount_comment'+meta_id).html('<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-up"></i><i class="socialwall_postlikecount_post ">'+totallikes+'&nbsp</i><i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-down"></i><i class="socialwall_postlikecount_post ">'+totaldislikes+'</i><span class="commented-date" style="color: #90949c;font-family: Arial;font-size:12px;">'+comment_date+'</span>');
		},
		error:function(data){alert(data);
...
function user_post_data(post_name,user_id,posttype){
var check;
if(isNaN(posttype)){
check='posttype';
}
...
check='puser_id';
	...
	{	
		...
		var str = 'action=post_userdata&file_name='+htmlDecode(post_content)+'&user_id='+user_id+'&'+check+'='+posttype;
		jQuery.ajax({
		url: ajaxurl,
		data: str,
		dataType:'json',
	    ...
	    jQuery(this).uploadFile({
	       ...
	       allowedTypes: allowed,
	        ...
	        onSuccess:function(files,data,xhr){
	            ...
	            jQuery('.upload').append('<textarea name="image_desc" class="image-desc" id="image_desc" placeholder="Enter photo description" rows="2" style="width: 100%;" cols="50"></textarea> <button type="submit"  name="post_photo_desc" id="post_photo_desc" value="Send Photo"><i class="fa fa-send fa-fw"></i><b>Share Photo</b></button>');
	            
	             jQuery('#post_photo_desc').click(function(){
	            	 var photo_desc = jQuery('#image_desc').val();
	            	 str = 'action=userwall_upload_img&filetype='+filetype+'&width='+width+'&height='+height+'&src='+src+'&media_type='+media_type+'&srcname='+srcname+'&media_name='+medianame+'&thumbnail_path='+thumbnail_path+'&photo_desc='+photo_desc+'&posttype='+posttype+'&url='+url;
	            	 jQuery.ajax({
	  	                url: ajaxurl,
	  	                data: str,
	  	                dataType: 'JSON',
