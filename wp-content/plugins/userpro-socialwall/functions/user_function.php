...
<div class="userwall" id=<?php echo $post->ID?>>
	...
	<?php echo $post->ID?>>
		<div class="displayname">
			...
			<div class="post-date">
			<?php _e("Posted On ","userpro-userwall");echo $new_date;?>
			</div>
		</div>
		<div class="clear"></div>
	...
	<div class="userwall-comment-data-<?php echo $post->ID;?>"
		...
		<div class="userwall_postlikecount_post"
			id=userwall_postlikecount_post <?php echo $post->ID?>>
			<i
				onclick="userwall_postlikecount_post(<?php echo $post->ID;?>,<?php echo get_current_user_id()?>,<?php if(empty($like_post)) echo 0; else echo  count($like_post);?>,<?php if (empty($dislike_post)) echo 0; else echo count($dislike_post);?>);"
				class="fa fa-thumbs-up socialwall_postlikecount_post btn-like-dislike"></i>
			<i class="socialwall_postlikecount_post "><?php if(empty($like_post)) echo 0; else echo  count($like_post);?>
			</i> <i
				onclick="userwall_postdislikecount_post(<?php echo $post->ID;?>,<?php echo get_current_user_id()?>,<?php if (empty($dislike_post)) echo 0; else echo count($dislike_post);?>,<?php if(empty($like_post)) echo 0; else echo  count($like_post);?>);"
				class="fa fa-thumbs-down socialwall_postlikecount_post btn-like-dislike"></i><i
	...
	foreach($results as $result)
	{
	   ...
	   $commenttime = date('d-M-Y', $commenttimestamp);?>
				<div class="post-date post-comment-date">
				<?php echo _e("Commented On ","userpro-userwall").$commenttime;?>
				</div>
				<?php
				...
				<div class="userwall_commentlikecount_comment"
					...
					if( $flag ){
						...
						<?php
						echo '<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-up"></i>'; ?>
					<i class="socialwall_postlikecount_post "><?php if(!isset($comment['like'])) echo 0; else echo  $comment['like'];?>
					</i>
					<?php
					echo '<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-down"></i>';
					?>
					<i class="socialwall_postlikecount_post "><?php if (!isset($comment['dislike'])) echo 0; else echo $comment['dislike'];?>
					</i>
					<?php
					}
					else{
						?>
					<i
						onclick="userwall_commentlikedislikecount_comment(<?php echo $result->meta_id;?>,<?php echo get_current_user_id()?>,<?php  if(!isset($comment['like'])) echo 0; else echo  $comment['like']?>,<?php if (!isset($comment['dislike'])) echo 0; else echo $comment['dislike'];?>,'like',<?php echo $post->ID;?>);"
						class="socialwall_postlikecount_post fa fa-thumbs-up btn-like-dislike"></i><i
					...
					?>
					<i
						onclick="userwall_commentlikedislikecount_comment(<?php echo $result->meta_id;?>,<?php echo get_current_user_id()?>,<?php  if (!isset($comment['like'])) echo 0; else echo $comment['like'];?>,<?php if(!isset($comment['dislike'])) echo 0; else echo  $comment['dislike'];?>,'dislike',<?php echo $post->ID;?>);"
						class="socialwall_postlikecount_post fa fa-thumbs-down btn-like-dislike"></i><i
	...
	?>
				...
				<div class="commenttext">
					...
					<textarea id=userwall-comment- <?php echo $post->ID;?>
						...
						onkeypress="user_post_comment(this,'userwall-comment',<?php echo get_current_user_id();?>,<?php  echo $post->ID;?>,event,<?php echo count_comment($post->ID);?>,<?php  echo $limit_comment;?>,<?php echo get_the_ID(); ?>);"
...
function display_notification($user_id){
	...
	if(is_array($results)){
		foreach ($results as $result){
			...
			$output .= '<div class="socail_notify_comment"><a href="'.$userpro->permalink($userdata->ID).'">'.ucfirst($userdata->user_login).'</a> has commented on your <a href="'.$link.'" target="_blank">post</a> <br></div><hr>';
		}
	}
