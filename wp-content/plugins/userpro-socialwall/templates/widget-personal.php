...
<div class="large-12 block columns">
	...
	<div class="textarea">
	  <div contentEditable="true" data-text="<?php _e('Leave your memory.','userpro-userwall');?>" id="userpost" class="userpost" style="border: 1px solid #ccc;width:auto; height: 72px;overflow: hidden;"></div>
	    ...
	    <div class="share_button">	<input type="button" class="userpro-button"  name="Post_Now" value="<?php _e('Share','userpro-userwall');?>" title="<?php _e('Add to Wall','userpro-userwall'); ?>" onclick="user_post_data('userpost',<?php echo get_current_user_id();?>,<?php echo $Personal_user_id;?> );">
...
<?php
    ...
    <i onclick="userwall_commentlikedislikecount_comment(<?php echo $result->meta_id;?>,<?php echo get_current_user_id()?>,<?php  if(!isset($comment['like'])) echo 0; else echo  $comment['like']?>,<?php if (!isset($comment['dislike'])) echo 0; else echo $comment['dislike'];?>,'like');"  class="socialwall_postlikecount_post fa fa-thumbs-up btn-like-dislike"></i><i class="socialwall_postlikecount_post " id="comment_like_count"><?php if(!isset($comment['like'])) echo 0; else echo  $comment['like'];?></i>
 
<?php if( $flag ){
 	echo '<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-down"></i>';
  }
 ?>
    <i onclick="userwall_commentlikedislikecount_comment(<?php echo $result->meta_id;?>,<?php echo get_current_user_id()?>,<?php  if (!isset($comment['like'])) echo 0; else echo $comment['like'];?>,<?php if(!isset($comment['dislike'])) echo 0; else echo  $comment['dislike'];?>,'dislike');"  class="socialwall_postlikecount_post fa fa-thumbs-down btn-like-dislike"></i><i class="socialwall_postlikecount_post" id="comment_dislike_count"><?php if (!isset($comment['dislike'])) echo 0; else echo $comment['dislike'];?></i>
<?php }?>
