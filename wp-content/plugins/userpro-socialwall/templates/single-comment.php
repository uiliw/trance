...
<?php 
  if( $flag ){
    ?>
<?php
echo '<i style="color:black;opacity: 0.5;" class="upc-thumb socialwall_postlikecount_post fa fa-thumbs-up"></i>'; ?>
...
?>
<i onclick="userwall_commentlikedislikecount_comment(<?php echo $meta_id;?>,<?php echo get_current_user_id()?>,<?php  if(!isset($comment['like'])) echo 0; else echo  $comment['like']?>,<?php if (!isset($comment['dislike'])) echo 0; else echo $comment['dislike'];?>,'like',<?php echo $post->ID;?>,<?php if (!isset($commenttimestamp)) echo 0; else echo $commenttimestamp;?>);"  class="socialwall_postlikecount_post fa fa-thumbs-up btn-like-dislike"></i><i class="socialwall_postlikecount_post " id="comment_like_count"><?php if(!isset($comment['like'])) echo 0; else echo  $comment['like'];?></i>
 <?php if( $flag ){
 	echo '<i style="color:black;opacity: 0.5;" class="socialwall_postlikecount_post fa fa-thumbs-down"></i>';
  }
 ?>
<i onclick="userwall_commentlikedislikecount_comment(<?php echo $meta_id;?>,<?php echo get_current_user_id()?>,<?php  if (!isset($comment['like'])) echo 0; else echo $comment['like'];?>,<?php if(!isset($comment['dislike'])) echo 0; else echo  $comment['dislike'];?>,'dislike',<?php echo $post->ID;?>,<?php if (!isset($commenttimestamp)) echo 0; else echo $commenttimestamp;?>);"  class="socialwall_postlikecount_post fa fa-thumbs-down btn-like-dislike"></i><i class="socialwall_postlikecount_post" id="comment_dislike_count"><?php if (!isset($comment['dislike'])) echo 0; else echo $comment['dislike'];?></i>
<?php }?>
