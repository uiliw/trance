...
class Revslider_Featured_Addon_Public {
	...
	private function create_slider( $html, $post_id , $post_thumbnail_id=0 ) {
		...
		if ( $revslider_featured_addon_values['revslider-featured-addon-type']=='auto' && !empty($revslider_featured_addon_values['revslider-featured-addon-slider']) ){
			...
			$html = do_shortcode('[rev_slider alias="'.$revslider_featured_addon_values['revslider-featured-addon-slider'].'"][gallery ids="0,'.$post_id.'" revslider_function="related_posts"][/rev_slider]');
		}
		else {
			if (!empty($post_revslider_id)){
				$html = do_shortcode('[rev_slider alias="'.$post_revslider_id.'"][gallery ids="0,'.$post_id.'" revslider_function="related_posts"][/rev_slider]');
			}
		}
