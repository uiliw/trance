...
class Rev_addon_rel_posts_Public {
	...
	public function filter_print_related_posts($content) {
		...
		if( empty($rev_slider_addon_values['rs-addon-rel-'.$post_type.'-slider']) ) return $content;

		// Check for Cached result
		$rev_slider_addon_transient_name = 'rev_addon_rel_posts_post_id_' . $post->ID;
		if ($rev_slider_addon_values['rs-addon-rel-'.$post_type.'-caching'] > 0 && false !== ($data = get_transient( $rev_slider_addon_transient_name)))
		...
		$related_posts_shortcode = do_shortcode('[rev_slider alias="'.$rev_slider_addon_values['rs-addon-rel-'.$post_type.'-slider'].'"][gallery ids="0,'.implode(",", $related_posts).'" revslider_function="related_posts"][/rev_slider]');
		// Filter
		...
		if( !empty($rev_slider_addon_values['rs-addon-rel-'.$post_type.'-position']) && $rev_slider_addon_values['rs-addon-rel-'.$post_type.'-position'] == 'top' ){
		...
		set_transient( $rev_slider_addon_transient_name, $related_posts_shortcode, $rev_slider_addon_values['rs-addon-rel-'.$post_type.'-caching'] );
	...
	public function get_related_posts($rev_slider_addon_values){
		...
		$related_posts = $this->get_related_posts_by_tax($post->ID,$rev_slider_addon_values['rs-addon-rel-'.$post_type.'-number'],'',$post_type,$rev_slider_addon_values['rs-addon-rel-'.$post_type.'-start-with']);

		// Number of Posts to display
		$needed_posts = $rev_slider_addon_values['rs-addon-rel-'.$post_type.'-number'] - sizeof($related_posts);
		...
		if(!is_array($related_posts) || $needed_posts){
			...
			switch ($rev_slider_addon_values['rs-addon-rel-'.$post_type.'-fill-with']) {
				...
				case 'popular':
					...
					break;
				default: // Taxonomy
					$related_posts = array_merge($related_posts,$this->get_related_posts_by_tax($post->ID,$needed_posts,$related_posts_list,$post_type,$rev_slider_addon_values['rs-addon-rel-'.$post_type.'-fill-with']));
					break;
			}
		}	
