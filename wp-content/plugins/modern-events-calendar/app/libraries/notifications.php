...
class MEC_notifications extends MEC_base
{
    ...
    public function admin_notification($book_id)
    {
        ...
        if(strpos($message, '%%attendee_full_info%%') !== false)
        {
            ...
            $attendee_full_info .= 'Name: '.((isset($attendee['name']) and trim($attendee['name'])) ? $attendee['name'] : '---')."\r\n";
            $attendee_full_info .= 'Email: '.((isset($attendee['email']) and trim($attendee['email'])) ? $attendee['email'] : '---')."\r\n";
