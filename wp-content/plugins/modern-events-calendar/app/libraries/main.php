...
class MEC_main extends MEC_base
{
    ...
    {
        $skins = array('list'=>__('List View', 'mec'), 'grid'=>__('Grid View', 'mec'), 'carousel'=>__('Carousel View', 'mec'), 'full_calendar'=>__('Full Calendar', 'mec'), 'monthly_view'=>__('Calendar/Monthly View', 'mec'), 'daily_view'=>__('Daily View', 'mec'), 'weekly_view'=>__('Weekly View', 'mec'), 'map'=>__('Map View', 'mec'), 'cover'=>__('Cover View', 'mec'), 'countdown'=>__('Countdown View', 'mec'));
    ...
    public function include_styles()
    {
        ...
        {
            echo '<style type="text/css">'.stripslashes($styles['CSS']).'</style>';
        }
    }
    ...
    public function get_marker_infowindow($marker)
    {
        ...
        <div class="mec-marker-infowindow-wp">
            ...
            <div class="mec-marker-infowindow-content">
                <span>'._n('Event', 'Events', $count, 'mec').' '.esc_html__('at this location', 'mec').'</span>
     ...
     * @return string
     */
    public function ical_URL($event_id)
    {
        $url = $this->URL('site');
     ...
     * @return string
     */
    public function ical_single($event_id)
    {
        // MEC Render Library
        $render = $this->getRender();
        ...
        $start_time = strtotime($dates[0]['start']['date'].' '.sprintf("%02d", $dates[0]['start']['hour']).':'.sprintf("%02d", $dates[0]['start']['minutes']).' '.$dates[0]['start']['ampm']);
        $end_time = strtotime($dates[0]['end']['date'].' '.sprintf("%02d", $dates[0]['end']['hour']).':'.sprintf("%02d", $dates[0]['end']['minutes']).' '.$dates[0]['end']['ampm']);
        ...
        $ical .= "DTSTART:".date('Ymd\\THi00\\Z', $start_time).PHP_EOL;
        $ical .= "DTEND:".date('Ymd\\THi00\\Z', $end_time).PHP_EOL;
        $ical .= "DTSTAMP:".date('Ymd\\THi00\\Z', $stamp).PHP_EOL;
     ...
     * @return string
     */
    public function get_event_date_permalink($url, $date = NULL)
    {
        ...
        if(!isset($settings['single_date_method']) or (isset($settings['single_date_method']) and $settings['single_date_method'] == 'next')) return $url;
     ...
     * @return array
     */
    public function get_bookings($event_id, $date)
    {
        $time = strtotime($date);
